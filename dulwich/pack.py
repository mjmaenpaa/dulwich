...
class PackStreamReader(object):
    ...
    def _read(self, read, size):
        ...
        if PY2:
            ...
            self._trailer.extend((int2byte(i) for i in data[-to_add:]))
...
class PackData(object):
    ...
    def get_object_at(self, offset):
        ...
        try:
            ...
            pass
        assert isinstance(offset, integer_types), 'offset was %r' % offset
...
class DeltaChainIterator(object):
    ...
    * crc32          (if _compute_crc32 is True)
    ...
    def _walk_ref_chains(self):
        ...
        for base_sha, pending in sorted(items(self._pending_ref)):
...
def write_pack(filename, objects, deltify=None, delta_window_size=None):
    ...
    entries = [(k, v[0], v[1]) for (k, v) in items(entries)]
    entries.sort()
