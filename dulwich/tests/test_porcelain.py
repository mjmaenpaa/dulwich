...
class RemoveTests(PorcelainTestCase):
    ...
    def test_remove_file(self):
        ...
        # TODO add accepts unicode, rm does not ????
        porcelain.add(self.repo.path, paths=["foo"])
...
class SymbolicRefTests(PorcelainTestCase):
    ...
    def test_set_symbolic_ref_other_than_master(self):
        c1, c2, c3 = build_commit_graph(self.repo.object_store, [[1], [2, 1],
            [3, 1, 2]], attrs=dict(refs=b'develop'))
        self.repo.refs[b'HEAD'] = c3.id
...
class PushTests(PorcelainTestCase):
    ...
    def test_simple(self):
        ...
        refs_path = os.path.join(b'refs', b'heads', b'foo')
...
class StatusTests(PorcelainTestCase):
    ...
    def test_status(self):
        ...
        self.assertEqual(results.staged['add'][0], filename_add.encode(sys.getfilesystemencoding()))
    ...
    def test_get_tree_changes_add(self):
        ...
        self.assertEqual(changes['add'][0], filename.encode(sys.getfilesystemencoding()))
    ...
    def test_get_tree_changes_modify(self):
        ...
        self.assertEqual(changes['modify'][0], filename.encode(sys.getfilesystemencoding()))
    ...
    def test_get_tree_changes_delete(self):
        ...
        self.assertEqual(changes['delete'][0], filename.encode(sys.getfilesystemencoding()))
        self.assertEqual(len(changes['add']), 0)
        self.assertEqual(len(changes['modify']), 0)
