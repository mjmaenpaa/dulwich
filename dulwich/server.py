...
class ProtocolGraphWalker(object):
    ...
    def determine_wants(self, heads):
        ...
        """
        values_heads = set(values(heads))
        if self.advertise_refs or not self.http_req:
            for i, (ref, sha) in enumerate(sorted(items(heads))):
                line = sha + b' ' + ref
                if not i:
...
class ReceivePackHandler(Handler):
    ...
    def handle(self):
        if self.advertise_refs or not self.http_req:
            refs = sorted(items(self.repo.get_refs()))
...
class TCPGitRequestHandler(SocketServer.StreamRequestHandler):
    ...
    def handle(self):
        ...
        # TODO(garyvdm) Review this
        # This (TCP server) args come in as bytes, but for http they come in as unicode.
        args[0] = args[0].decode(sys.getfilesystemencoding())
...
def serve_command(handler_cls, argv=sys.argv, backend=None, inf=sys.stdin,
    ...
    """
    write = outf.buffer.write if isinstance(outf, io.TextIOBase) else outf.write
    read = inf.buffer.read if isinstance(inf, io.TextIOBase) else inf.read
...
def generate_objects_info_packs(repo):
    ...
    for pack in repo.object_store.packs:
        yield b'P ' + pack.data.filename.encode('ascii') + b'\n'
